...
from models.model_clam_multilabel import CLAM_MB, CLAM_SB
...
def train(datasets, cur, args):
    ...
    if args.model_type in ['clam_sb', 'clam_mb']:
        model_dict.update({'gate': args.gate})
...
def train_loop(epoch, model, loader, optimizer, n_classes, n_labels, writer = None, loss_fn = None):   
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        train_loss += loss_value
        if (batch_idx + 1) % 20 == 0:
            print('batch {}, loss: {:.4f}, label: {}, bag_size: {}'.format(batch_idx, loss_value, label.cpu().detach().numpy(), data.size(0)))
