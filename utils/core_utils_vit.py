...
from models.model_local_vit import LocalViT
from models.model_cluster_vit import ClusterViT
...
def train(datasets, cur, args):
    ...
    if args.model_type == 'local_vit':
        model_dict = {"dropout": args.drop_out, 'n_classes': args.n_classes}
        model = LocalViT(**model_dict)
    elif args.model_type == 'cluster_vit':
        model_dict = {"cluster_coeff_list": [100, 50], "dropout": args.drop_out, 'n_classes': args.n_classes}
        model = ClusterViT(**model_dict)
    ...
    print('\nInit Loaders...', end=' ')
    train_loader = get_split_loader(train_split, training=True, testing = args.testing, weighted = args.weighted_sample, collate=None)
    val_loader = get_split_loader(val_split,  testing = args.testing, collate=None)
    test_loader = get_split_loader(test_split, testing = args.testing, collate=None)
    print('Done!')
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer = None, loss_fn = None):
    ...
    print('\n')
    for batch_idx, (data, label, knn) in enumerate(loader):
        data, label, knn = data.to(device), label.to(device), knn.to(device)
        logits, Y_prob, Y_hat = model(data, knn)
