...
from models.model_vos_multilabel_generic import VOS_MB, VOS_SB
...
def test(test_split, cur, args):
    ...
    print('\nInit Model...', end=' ')
    model_dict = {"dropout": args.drop_out, 'n_classes': args.n_classes, 'n_labels': args.n_labels, 'att_net': args.att_model_type}
    ...
    print('\nInit Loader...', end=' ')
    collate = 'ml_extended' if args.att_model_type == 'LocalViT' else 'ml'
    test_loader = get_split_loader_multilabel(test_split, testing = args.testing, collate= collate)
    print('Done!')
    ...
    for batch_idx, data in enumerate(test_loader):
        if args.att_model_type == 'LocalViT':
            data, label, knn = data[0].to(device), data[1].to(device), data[2].to(device)
            data = (data, knn)
        else:
            data, label, coords = data[0].to(device), data[1].to(device), data[2].to(device)

        with torch.no_grad():
            logits, Y_prob, Y_hat, A_raw, _ = model(data)
        A = F.softmax(A_raw, dim=1)  # softmax over N
        for j in range(args.n_labels):
            top_p_ids = torch.topk(A[j:j+1, :], args.B)[1][-1]
            top_p = torch.index_select(data, dim=0, index=top_p_ids)
    ...
    # val_results_label_list, val_prob_label_dict_label_list = prepare_reports(model, val_loader, args.n_classes, args.labels, group = 'val', writer = writer, att_model_type = args.att_model_type)
    # test_results_label_list, test_prob_label_dict_label_list = prepare_reports(model, test_loader, args.n_classes, args.labels, group = 'test', writer = writer, att_model_type = args.att_model_type)
...
def train(datasets, cur, args):
    ...
    print('\nInit Loaders...', end=' ')
    collate = 'ml_extended' if args.att_model_type == 'LocalViT' else 'ml'
    train_loader = get_split_loader_multilabel(train_split, training=True, testing = args.testing, collate= collate)
    val_loader = get_split_loader_multilabel(val_split,  testing = args.testing, collate= collate)
    test_loader = get_split_loader_multilabel(test_split, testing = args.testing, collate= collate)
    print('Done!')
    ...
    for epoch in range(args.max_epochs):
        if not args.no_inst_module:     
            train_loop_clam(epoch, model, train_loader, optimizer, args.n_classes, args.n_labels, args.bag_weight, writer, loss_fn, args.att_model_type)
            stop = validate_clam(cur, epoch, model, val_loader, args.n_classes, args.n_labels,
                early_stopping, writer, loss_fn, args.results_dir, args.att_model_type)
...
def prepare_reports(model, loader, n_classes, labels, group = 'test', writer = None, att_model_type = 'clam'):
    n_labels = len(labels)
    patient_results, error, auc, acc_logger, slide_probs, slide_labels = summary(model, loader, n_classes, n_labels, att_model_type)
...
def train_loop_clam(epoch, model, loader, optimizer, n_classes, n_labels, bag_weight, writer = None, loss_fn = None, att_model_type = 'clam'):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        logits, Y_prob, Y_hat, _, instance_dict = model.forward_virtual(data, labels=label, instance_eval=True)
        ...
        if (batch_idx + 1) % 20 == 0:
            print('batch {}, loss: {:.4f}, instance_loss: {:.4f}, weighted_loss: {:.4f}, '.format(batch_idx, loss_value, instance_loss_value, total_loss.item()) + 
                'label: {}, bag_size: {}'.format(label.cpu().detach().numpy(), bag_size))
            break
...
def validate_clam(cur, epoch, model, loader, n_classes, n_labels, early_stopping = None, writer = None, loss_fn = None, results_dir = None, att_model_type = 'clam'):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    acc_logger = Accuracy_Logger_Multilabel(n_classes=n_classes, n_labels=n_labels)
    inst_logger = Accuracy_Logger(n_classes=n_classes)
...
def summary(model, loader, n_classes, n_labels, att_model_type = 'clam'):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    acc_logger = Accuracy_Logger_Multilabel(n_classes=n_classes, n_labels=n_labels)
    model.eval()
    test_loss = 0.
