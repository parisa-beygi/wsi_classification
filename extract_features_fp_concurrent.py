...
if __name__ == '__main__':
	...
	if args.slide_idx != -1:
		slide_names = select_slides(slide_names, args.slide_idx, num_workers)

	total = len(slide_names)

	prefix = f'Processing {total} slides: '

	cur_slide_names = slide_names
	...
	for slide_args in slides_args:
		futures.append(executor.submit(compute_w_loader, slide_args[0], slide_args[1], slide_args[2], model, args.batch_size, 1, 20, True, args.custom_downsample, args.target_patch_size))
	results_to_write = concurrent.futures.wait(futures)		
