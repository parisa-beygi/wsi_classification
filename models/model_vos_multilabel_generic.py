...
"""
args:
    att_net: the attention network to use for computing attention scores
...
class VOS_SB(nn.Module):
    def __init__(self, att_net = 'clam_gated', size_arg = "small", dropout = False,  n_classes=2, n_labels = 2,
        ...
        # for each label, maintaining a data queue for each class
        self.data_dict = torch.zeros(n_labels, n_classes, sample_number, size[1]).cuda()
        # keep the track of the number of instances in the queue for each class for each label  
        self.number_dict = {j: {i: 0 for i in range(n_classes)} for j in range(n_labels)}

        # for log_sum_exp computation
        weight_energy = [torch.nn.Linear(n_classes, 1) for j in range(n_labels)]
        for we in weight_energy:
            torch.nn.init.uniform_(we.weight)
        self.weight_energy = nn.ModuleList(weight_energy)



        # instance-level classifier
        instance_classifier = [nn.Linear(size[1], 2) for j in range(n_labels)]
        self.instance_classifier = nn.ModuleList(instance_classifier)

        # logistic regression for separating energies of ID and OOD data
        energy_regression = [torch.nn.Linear(1, 2) for j in range(n_labels)]
        self.energy_regression = nn.ModuleList(energy_regression)
    ...
    def log_sum_exp(self, j, value, dim=None, keepdim=False):
        """
        ...
        if dim is not None:
            ...
            mult = F.relu(self.weight_energy[j].weight)
            sum_exp = torch.sum(
    ...
    def forward(self, h, labels=None, instance_eval=False, return_features=False, attention_only=False):
        device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ...
        if instance_eval:
            ...
            for j, label in enumerate(labels):
                ...
                for i in range(self.n_classes):
                    if i == 0:
                        X = self.data_dict[j][i] - self.data_dict[j][i].mean(0) #deduct the mean of self.sample_number in class i from each emb in class i
                        mean_embed_id = self.data_dict[j][i].mean(0).view(1, -1)
                    else:
                        X = torch.cat((X, self.data_dict[j][i] - self.data_dict[j][i].mean(0)), 0)
                        mean_embed_id = torch.cat((mean_embed_id,
                                                self.data_dict[j][i].mean(0).view(1, -1)), 0)
                ...
                ######################################################################
                # Estimate distribution for every class and sample ood for every class
                ######################################################################
                ...
                ########################################################################
                # Estimate distribution only for the current class and sample ood for it
                ########################################################################
                new_dis = torch.distributions.multivariate_normal.MultivariateNormal(
                    mean_embed_id[label], covariance_matrix=temp_precision)
                # new_dis = torch.distributions.multivariate_normal.MultivariateNormal(
                #     torch.zeros(mean_embed_id[index].shape).cuda(), covariance_matrix=torch.eye(temp_precision.shape[0]).cuda())
                ...
                cur_samples, index_prob = torch.topk(-prob_density, self.k_sample_ood)
                ood_samples = negative_samples[index_prob]
                ## ood_samples.shape == [self.k_sample_ood, 512]



                if len(ood_samples) != 0:
                    predictions_id = self.instance_classifier[j](self.data_dict[j][label][-2*self.k_sample:]) # predictions_id: [self.k_sample, 2]
                    energy_score_for_fg = self.log_sum_exp(j, predictions_id, 1) ## energy_score_for_fg.shape == [self.k_sample]
                    # debug:
                    predictions_ood = self.instance_classifier[j](ood_samples) ## predictions_ood.shape == [2*self.k_sample_ood, 2]
                    # predictions_ood = self.instance_classifier(torch.rand(ood_samples.shape).cuda()) ## predictions_ood.shape == [2*self.k_sample_ood, 2]

                    # debug:
                    energy_score_for_bg = self.log_sum_exp(j, predictions_ood, 1) ## energy_score_for_bg.shape == [2*self.k_sample_ood]
                    ...
                    labels_for_lr = torch.cat((torch.ones(2*self.k_sample).cuda(),
                                            torch.zeros(len(ood_samples)).cuda()), -1) # [self.k_sample + 2*self.k_sample_ood]

                    energy_pred = self.energy_regression[j](input_for_lr.view(-1, 1)) # energy_pred.shape == [self.k_sample + 2*self.k_sample_ood, 2]
                    # debug:
        ...
        M = self.attention_net.compute_agg(A, h)
    ...
    def forward_virtual(self, h, labels=None, epoch = 0, instance_eval=False, return_features=False, attention_only=False):
        device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
        if self.att_net_type == 'LocalViT':
            A, h = self.attention_net(h[0], h[1])  # NxK (N: number of patches in slide, K: number of classes in attention (=1)), h: N x 512 
            A = A.repeat(1, self.n_labels)
            ...
            for j, label in enumerate(labels):
                ...
                if sum_temp == self.n_classes*self.sample_number and epoch < self.start_epoch:
                    ...
                    if torch.isnan(top_p).any():
                        print ('here')
                    self.data_dict[j][label] = torch.cat((self.data_dict[j][label][self.k_sample:], top_p.detach()), 0)
                    ...
                    sample_temp = self.number_dict[j][label]
                    if sample_temp < self.sample_number:
                        ...
                        if torch.isnan(top_p[:num_add]).any():
                            print ('here')
                        self.data_dict[j][label][sample_temp: sample_temp + num_add] = top_p[:num_add].detach()
                        self.number_dict[j][label] += num_add
                    else:
        ...
        if loss_count:
            instance_loss = instance_loss / loss_count
        # M = torch.mm(A, h)  # M.shape == [1, 512]
...
args:
    ...
    instance_loss_fn: loss function to differentiate ID and OOD data 
"""
class VOS_MB(nn.Module):
    def __init__(self, att_net = 'clam_gated', size_arg = "small", dropout = False,  n_classes=2, n_labels = 2,
                k_sample=8, sample_number = 1000, k_sample_ood = 4, sample_from = 10000, instance_loss_fn=nn.CrossEntropyLoss(), start_epoch = 0):
        super(VOS_MB, self).__init__()
        self.size_dict = {"small": [1024, 512, 256], "big": [1024, 512, 384]}
        size = self.size_dict[size_arg]
        fc = [nn.Linear(size[0], size[1]), nn.ReLU()]
        if dropout:
    ...
    def forward(self, h, labels=None, instance_eval=False, return_features=False, attention_only=False):
        ...
        if instance_eval:
            ...
            for j, label in enumerate(labels):
                # (1) get the top attended (k_sample) patches in h
                top_p_ids = torch.topk(A[j:j+1, :], self.k_sample)[1][-1]
                top_p = torch.index_select(h, dim=0, index=top_p_ids)
                # (2) get the total number of samples in the class queues
