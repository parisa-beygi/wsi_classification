...
class VOS_MB(nn.Module):
    def __init__(self, gate = True, size_arg = "small", dropout = False,  n_classes=2, n_labels = 2,
        ...
        if gate:
            attention_net = Attn_Net_Gated(L = size[1], D = size[2], dropout = dropout, n_classes = n_labels)
        else:
            attention_net = Attn_Net(L = size[1], D = size[2], dropout = dropout, n_classes = n_labels)
        fc.append(attention_net)
        self.attention_net = nn.Sequential(*fc)
        bag_classifiers = [nn.Linear(size[1], 1) for i in range(n_labels)] #use an indepdent linear layer to predict each class
        ...
        if instance_eval:
            ...
            for j, label in enumerate(labels):
                ...
                if len(ood_samples) != 0:
                    predictions_id = self.instance_classifier[j](top_p) # predictions_id: [self.k_sample, 2]
    ...
    def forward_virtual(self, h, labels=None, epoch = 0, instance_eval=False, return_features=False, attention_only=False):
        ...
        if torch.isnan(h).any():
            print ('here')
            A_bug, h_bug = self.attention_net(h_prev)
