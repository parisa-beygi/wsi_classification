...
def plot_tsne(n_clusters, slide_id, X, labels, dir):
	m = TSNE(learning_rate=50)
	feat_cols = [ 'f'+str(i) for i in range(X.shape[1]) ]
	data = pd.DataFrame(X,columns=feat_cols)
	tsne_features = m.fit_transform(data)
	data['x'] = tsne_features[:, 0]
	data['y'] = tsne_features[:, 1]
	data['label'] = list(map(lambda x: str(x), labels))

	sns.scatterplot(x="x", y="y", data=data, hue="label", palette=sns.color_palette("Set1", data.label.nunique()), legend=False)
	plt.title(f"n_clusters = {n_clusters}")

	plt.savefig(os.path.join(dir, f'{slide_id}.png'))
	plt.clf()
...
class Generic_WSI_Classification_Dataset(Dataset):
	...
	def get_split_from_df(self, all_splits, split_key='train'):
		...
		if len(split) > 0:
			...
			split = Generic_Split(df_slice, data_dir=self.data_dir, num_classes=self.num_classes, processed_dir=self.processed_dir)
		else:
			split = None
	...
	def return_splits(self, from_id=True, csv_path=None):
		...
		if from_id:
			if len(self.train_ids) > 0:
				...
				train_split = Generic_Split(train_data, data_dir=self.data_dir, num_classes=self.num_classes, processed_dir=self.processed_dir)
			...
			if len(self.val_ids) > 0:
				...
				val_split = Generic_Split(val_data, data_dir=self.data_dir, num_classes=self.num_classes, processed_dir=self.processed_dir)
			...
			if len(self.test_ids) > 0:
				...
				test_split = Generic_Split(test_data, data_dir=self.data_dir, num_classes=self.num_classes, processed_dir=self.processed_dir)
...
class Generic_Graph_Dataset(Generic_WSI_Classification_Dataset):
	...
	def check_processed(self):
		...
		for idx in range(len(self.slide_data)):
			...
			file_name = f'data_{slide_id}.pt'

			if file_name not in os.listdir(self.processed_dir):
				return False
		
		return True
		
		
	def create_wheel_edges(self, centroid_index, wheel_num_points, begin):
	...
	def construct_graph(self, slide_id, data):
		# data : n, d
		...
		cents, labels = KMeans_clustering_func(data, k=n_clusters)

		# plot_tsne(n_clusters, slide_id, data, labels, self.plot_dir)
		centroids = []
		perpheries = []
		periphery_nums = []
		...
		for i, c in enumerate(cents):
			...
			periphery_nums.append(min(len(dist), self.k_farthes_in_cluster))
		...
		start_index = len(centroids)
		for i in range(len(centroids)):
			pnodes.append(perpheries[i])

			num = len(perpheries[i])
			wheel_edges = self.create_wheel_edges(i, num, start_index)
			start_index += num
	...
	def process(self):
		...
		for idx in range(data_size):
			...
			all_nodes, all_edges = self.construct_graph(slide_id, features)
		...
		print (f'Processed all {count} slides!')

	def __getitem__(self, idx):
		slide_id = self.slide_data['slide_id'][idx]
		data = torch.load(os.path.join(self.processed_dir, f'data_{slide_id}.pt'))
		return data


class Generic_Split(Generic_Graph_Dataset):
	def __init__(self, slide_data, data_dir=None, num_classes=2, processed_dir='root'):
		self.slide_data = slide_data
		self.data_dir = data_dir
