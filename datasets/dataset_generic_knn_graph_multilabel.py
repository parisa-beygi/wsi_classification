...
class Generic_KNN_Graph_Dataset(Generic_WSI_Classification_Dataset):
	def __init__(self, data_dir, k_neighbors, **kwargs):	
		super(Generic_KNN_Graph_Dataset, self).__init__(**kwargs)
	...
	# def get_knn(self, coords):
	# 	dist = distance_matrix(coords, coords) # n, n
	# 	knn_indices = np.argpartition(dist, self.k_neighbors, axis = 1)[:, :self.k_neighbors] # n, k
	# 	knn = dist[np.arange(dist.shape[0])[:, None], knn_indices] # n, k
	# 	s = np.argsort(knn, axis = 1) # n, k
	# 	return knn_indices[np.arange(knn_indices.shape[0])[:, None], s]
	

	def get_knn(self, coords):
		n_neighbors=min(self.k_neighbors, len(coords))
		nbrs = NearestNeighbors(n_neighbors=n_neighbors).fit(coords)
		distances, indices = nbrs.kneighbors(coords)
		is_sorted = lambda a: np.all(a[:-1] <= a[1:])
		assert np.array(list(map(lambda l: is_sorted(l), distances))).all(), "KNN return unsorted neighbors!"
		return indices
		

	def __getitem__(self, idx):
		slide_id = self.slide_data['slide_id'][idx]
		label = np.array(list(self.slide_data[self.label_col].loc[idx]))
		...
		if not self.use_h5:
			...
			knn = self.get_knn(coords)
			knn = torch.from_numpy(knn)

			return features, label, knn


class Generic_Split(Generic_KNN_Graph_Dataset):
	def __init__(self, slide_data, label_col, data_dir=None, num_classes=2, k_neighbors=64):
		self.use_h5 = True
		self.slide_data = slide_data
		self.data_dir = data_dir
