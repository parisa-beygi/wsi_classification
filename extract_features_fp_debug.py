...
from models.model_KimiaNet import KimiaNet
import argparse
...
# model selection specific options
parser.add_argument('--model_name', type=str, default= 'resnet50_baseline')
...
if __name__ == '__main__':
	...
	if args.model_name =='resnet50_baseline':
		model = resnet50_baseline(pretrained=True)
	elif args.model_name =='kimianet':
		model = KimiaNet(weights_location=args.model_weight_path, feature_extracting=True)
	model = model.to(device)
	
	# print_network(model)
	print (f'number of devices: {torch.cuda.device_count()}')
	print (f'device: {device}')
	if torch.cuda.device_count() > 1 and args.model_name == 'resnet50_baseline':
