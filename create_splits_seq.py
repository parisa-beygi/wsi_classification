...
parser.add_argument('--pretrain_frac', type=float, default= 0.0,
                    help='fraction of labels for pretrain (default: 0.1)')
...
pretrain_num = np.round(num_slides_cls * args.pretrain_frac).astype(int) if args.pretrain_frac else None
columns = ['pretrain', 'train', 'val', 'test'] if args.pretrain_frac else ['train', 'val', 'test']
...
if __name__ == '__main__':
    ...
    for lf in label_fracs:
        split_dir = 'splits/'+ str(args.task) + '_{}'.format(int(lf * 100)) + '_k{}'.format(args.k) + '_s{}'.format(args.seed)
        os.makedirs(split_dir, exist_ok=True)
        dataset.create_splits(k = args.k, pretrain_num = pretrain_num, val_num = val_num, test_num = test_num, label_frac=lf)
